...
trb_type = "TrbH"
file_path = f"C:\\Users\\churk\\OneDrive\\Documents\\GeorgiaTech\\Aero6361\\Project3\\AE6361_Project3\\Project_3_NPSS_Release1\\src\\{trb_type}.map"
...
def parse_table(text):
    ...
    for udx, table_match in enumerate(table_matches):
        ...
        alpha_pattern = re.compile(r'\s+PgeomDesign\s*=\s*([\d.]+)\s*\{')
        ...
        for idx, alpha_match in enumerate(alpha_matches):
            ...
            ncorr_pattern = re.compile(r'NcDes\s*=\s*([\d.]+)\s*\{')
            ...
            for jdx, ncorr_match in enumerate(ncorr_matches):
                ...
                rline_match = re.search(r'PRdes\s*=\s*(\*|{[^}]+})', ncorr_block)
                if rline_match:
...
# ── Plot ─────────────────────────────────────────────────────────────────────
# ── Settings ──────────────────────────────────────────────────────────────────
PgeomDesign_val = 1.0 # 1.0, 2.0
NCORR_STEP      = 1        # plot every Nth speed line (1 = all, 2 = every other, etc.)
EFF_CONTOUR_LEVELS = np.linspace(0.70,1.00,11)

# ── Extract data ─────────────────────────────────────────────────────────────
wc_data  = TB['TB_Wp' ][PgeomDesign_val]
eff_data = TB['TB_eff'][PgeomDesign_val]

# Common NcorrMap values across all three tables
ncorr_vals = sorted(set(wc_data.keys()) & set(eff_data.keys()))
...
for ncorr in ncorr_vals:
    if trb_type=="TrbL":
        ...
        N = 8600
    wc_nc_list  = np.array(wc_data[ncorr]['WcMap'])*(ncorr/100)*N # concerned with Wc*Nc with Trb Map
    wc_list  = wc_data[ncorr]['WcMap']
    pr_list  = wc_data[ncorr]['PRdes']
    eff_list = eff_data[ncorr]['effMap']
    min_len  = min(len(wc_list), len(pr_list), len(eff_list))
    all_wc_nc .extend(wc_nc_list [:min_len])
    all_wc .extend(wc_list [:min_len])
    all_pr .extend(pr_list [:min_len])
    all_eff.extend(eff_list[:min_len])

all_wc_nc  = np.array(all_wc_nc)
...
wc_nc_grid  = np.linspace(all_wc_nc.min(), all_wc_nc.max(), 300)
pr_grid     = np.linspace(all_pr.min(), all_pr.max(), 300)
WCNC, PR    = np.meshgrid(wc_nc_grid, pr_grid)
EFF_GRID    = griddata((all_wc_nc, all_pr), all_eff, (WCNC, PR), method='linear')
...
fig.add_trace(go.Contour(
    ...
    opacity=0.55,
    contours=dict(
        #start=0.50, end=0.90, size=(0.90 - 0.50) / 30,
...
# ── 2. Speed lines (Wcorr vs PR for each NcorrMap) ───────────────────────────
for idx, ncorr in enumerate(ncorr_vals):
    wc_nc_list = np.array(wc_data[ncorr]['WcMap'])*(ncorr/100)*N
    pr_list = wc_data[ncorr]['PRdes']
    min_len = min(len(wc_list), len(pr_list))
    color   = rainbow_color(idx,len(ncorr_vals))
    show    = True

    fig.add_trace(go.Scatter(
        x=wc_nc_list[:min_len],
        y=pr_list[:min_len],
        mode='lines+text',
...
fig.update_layout(
    title=dict(
        text=f'{trb_type} Map  (PgeomDesign = {PgeomDesign_val})',
        font=dict(size=24, family='Arial Black',color="black"),
        x=0.45,
        y=0.92,
    ),
    xaxis=dict(
        title=dict(text='Wc * Nc (lbm/s * RPM)', font=dict(size=22)),
        tickfont=dict(family="Arial Black",size=14,color="black"),
        range=[all_wc_nc.min() * 0.98, all_wc_nc.max() * 1.02],
...
fig.write_html(f'{trb_type}_map.html')
fig.show()
print(f"{trb_type} map saved to {trb_type}_map.html")
